...
/*   Updated: 2022/03/16 17:44:29 by seb              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_print_int(va_list ap, t_f_flags f_flags)
{
	char	*str;
	int		len;
	int		dec;
	...
	if (dec < 0)
	{
		...
		len += ft_format_before(i_c_flag, f_flags, len - 1);
	}
	else
		len += ft_format_before(i_c_flag, f_flags, len);
	if (dec < 0)
